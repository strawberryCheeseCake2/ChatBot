...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
	...
	func showVC(loggedIn: Bool) {
		...
		let navigationController = StyledNavigationController(rootViewController: homeVC)
		let tabBarController = StyledTabBarController()
		configerTabBarController()
		
		window?.rootViewController = tabBarController
		UIView.transition(with: window!, duration: 0.3, options: .transitionCrossDissolve) {
		}
		
		func configerTabBarController() {
